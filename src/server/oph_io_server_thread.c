...
int oph_io_server_free_status(oph_io_server_thread_status *status){
  ...
  if(status->curr_stmt != NULL){
    if(status->curr_stmt->partial_result_set != NULL)	oph_iostore_destroy_frag_recordset(&status->curr_stmt->partial_result_set);
    if(status->curr_stmt->device != NULL)	free(status->curr_stmt->device);
    if(status->curr_stmt->frag != NULL)	free(status->curr_stmt->frag);
...
gettimeofday(&start_time, NULL);
    ...
    if (ufds[0].revents != POLLIN){
      ...
      if(STRCMP(header, OPH_IO_SERVER_MSG_PING) == 0){
        ...
        if(global_status.device) free(global_status.device);
		  ...
		  for(j = 0; j < num_fields; j++){
		    ...
		    if(global_status.last_result_set->field_type[j] != OPH_IOSTORE_STRING_TYPE){
		      ...
		      if(global_status.last_result_set->field_type[j] == OPH_IOSTORE_LONG_TYPE){
		        snprintf(buffer, OPH_IO_SERVER_MAX_LONG_LEN, "%llu", *((unsigned long long *)global_status.last_result_set->record_set[i]->field[j]));
		      }
		      else{
