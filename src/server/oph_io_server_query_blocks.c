					...
					unsigned long long *loaded_frag_size)
{
	...
	if (_oph_ioserver_nc_read
	    (src_path, measure, row_num, frag_start, compressed_flag, dim_list_num, dims_type, dims_index, dims_start, dims_end, dim_unlim, sub_operation, sub_args, record_sets, &frag_size)) {
					...
					unsigned long long *loaded_frag_size)
{
	...
	//Check if number of dimension values are compliant
	if (tmpdim_list_num != dim_list_num) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_IO_SERVER_LOG_QUERY_MULTIVAL_ARGS_DIFFER, OPH_QUERY_ENGINE_LANG_OP_ESDM_IMPORT);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_IO_SERVER_LOG_QUERY_MULTIVAL_ARGS_DIFFER, OPH_QUERY_ENGINE_LANG_OP_ESDM_IMPORT);
		oph_iostore_destroy_frag_recordset(&record_sets);
		free(dims_type);
		free(dim_index_list);
		return OPH_IO_SERVER_EXEC_ERROR;
