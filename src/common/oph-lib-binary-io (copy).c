...
/*
		...
		do {
			...
			--retries;
			*shm_id = shm_open(name, O_CREAT | O_EXCL | O_RDWR, S_IRUSR | S_IWUSR);
			if ((*shm_id) > 0){
					ftruncate(*shm_id, sizeof_num*num_values*sizeof(char));
					return OPH_IOB_OK;
			}
