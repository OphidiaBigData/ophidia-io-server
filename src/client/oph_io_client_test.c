...
int main(int argc, char *argv[])
{
	...
	for (ii = 0; ii < nchildren; ii++) {
		if ((pid = fork()) == 0) {	/* child */
			...
			if ((res = oph_io_client_connect(host, port, NULL, "memory", &connection))) {
				...
				if ((res =
				     oph_io_client_setup_query(connection, "operation=create_frag;frag_name=trial1;column_name=id|measure;column_type=long|blob;", "memory", 0,
							       (oph_io_client_query_arg **) NULL, &stmt))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error %d in setup query '%s'\n", res,
					      "operation=create_frag;frag_name=trial1;column_name=id|measure;column_type=long|blob;");
					oph_io_client_free_query(stmt);
					res = oph_io_client_close(connection);
					return 0;
				}
				...
				int tot_run = 100;
				if ((res =
				     oph_io_client_setup_query(connection, "operation=insert;frag_name=trial1;field=id|measure;value=?|?;", "memory", tot_run, (oph_io_client_query_arg **) args,
							       &stmt))) {
				...
				//Create empty fragment
				if ((res =
				     oph_io_client_setup_query(connection, "operation=create_frag;frag_name=trial2;column_name=id|measure;column_type=long|blob;", "memory", 0,
							       (oph_io_client_query_arg **) NULL, &stmt))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error %d in setup query '%s'\n", res,
					      "operation=create_frag;frag_name=trial2;column_name=id|measure;column_type=long|blob;");
					oph_io_client_free_query(stmt);
					res = oph_io_client_close(connection);
					return 0;
				}
				...
				//Create empty fragment
				if ((res =
				     oph_io_client_setup_query(connection, "operation=insert;frag_name=trial1;field=id|measure;value=1|test;", "memory", 0, (oph_io_client_query_arg **) NULL,
							       &stmt))) {
				...
				if ((res =
				     oph_io_client_setup_query(connection, "operation=select;field=oph_dump('oph_double','oph_double',measure);from=trial1;", "memory", 0,
							       (oph_io_client_query_arg **) NULL, &stmt))) {
				...
				if ((res = oph_io_client_execute_query(connection, stmt))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error %d in executing query '%s'\n", res,
					      "operation=select;field=oph_dump('oph_double','oph_double',measure);from=trial1;");
					oph_io_client_free_query(stmt);
					res = oph_io_client_close(connection);
					return 0;
				}
				...
				if ((res =
				     oph_io_client_setup_query(connection, "operation=create_frag_select;frag_name=trial3;field=id|oph_reduce(measure);alias=id|measure;from=trial1;", "memory", 0,
							       (oph_io_client_query_arg **) NULL, &stmt))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error %d in setup query '%s'\n", res,
					      "operation=create_frag_select;frag_name=trial3;field=id|oph_reduce(measure);alias=id|measure;from=trial1;", "memory");
					oph_io_client_free_query(stmt);
					res = oph_io_client_close(connection);
					return 0;
				}
				...
				//Execute oph_dump plugin
				if ((res =
				     oph_io_client_setup_query(connection, "operation=select;field=oph_dump('oph_double','oph_double',measure);from=trial3;", "memory", 0,
							       (oph_io_client_query_arg **) NULL, &stmt))) {
				...
				if ((res = oph_io_client_execute_query(connection, stmt))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error %d in executing query '%s'\n", res,
					      "operation=select;field=oph_dump('oph_double','oph_double',measure);from=trial3;");
					oph_io_client_free_query(stmt);
					res = oph_io_client_close(connection);
					return 0;
				}
	...
	if (errno != ECHILD)
		pmesg(LOG_ERROR, __FILE__, __LINE__, "wait error");
