			      ...
			      oph_io_client_query ** query)
{
	...
	*query = (oph_io_client_query *) malloc(1 * sizeof(oph_io_client_query));
	...
	if (!args) {
		...
		message_len =
		    strlen(OPH_IO_CLIENT_MSG_EXEC_QUERY) + 1 + sizeof(unsigned int) + 2 * sizeof(unsigned long long) + strlen(operation) + 1 + strlen(operation) + 1 + 2 * sizeof(unsigned long long);
	...
	if (!query->args) {
		...
		for (n = 0; n < query->args_count; n++) {
			...
			switch (query->args[n]->arg_type) {
				case OPH_IO_CLIENT_TYPE_DECIMAL:
				case OPH_IO_CLIENT_TYPE_LONG:
				case OPH_IO_CLIENT_TYPE_LONGLONG:
					m += snprintf(query->query + m, strlen(OPH_IO_CLIENT_MSG_ARG_DATA_LONG) + 1, OPH_IO_CLIENT_MSG_ARG_DATA_LONG);
					break;
				case OPH_IO_CLIENT_TYPE_FLOAT:
				case OPH_IO_CLIENT_TYPE_DOUBLE:
				...
				case OPH_IO_CLIENT_TYPE_VARCHAR:
					...
					break;
				case OPH_IO_CLIENT_TYPE_LONG_BLOB:
				case OPH_IO_CLIENT_TYPE_BLOB:
				case OPH_IO_CLIENT_TYPE_BIT:
	...
	if (num_rows > 0) {
		...
		for (i = 0; i < num_rows; i++) {
			...
			for (j = 0; j < num_fields; j++) {
				...
				if ((*result_set)->max_field_length[j] < (*result_set)->result_set[i]->field_length[j])
					(*result_set)->max_field_length[j] = (*result_set)->result_set[i]->field_length[j];
			}
		}
		free(sub_reply);
		free(reply);
	...
	if (result->result_set) {
		for (i = 0; i < result->num_rows; i++) {
			if (result->result_set[i]) {
				if (result->result_set[i]->field_length)
					free(result->result_set[i]->field_length);
				for (j = 0; j < result->num_fields; j++) {
					if (result->result_set[i]->field[j])
						free(result->result_set[i]->field[j]);
				}
				if (result->result_set[i]->field)
					free(result->result_set[i]->field);
