...
char *multival_strchr(char *str)
{
	...
	for (i = 0; str[i]; i++) {
		...
		if (str[i] == OPH_QUERY_ENGINE_LANG_STRING_DELIMITER && !string_flag)
			string_flag = 1;
		//Count separator only if not in the middle of a string
		else if (str[i] == OPH_QUERY_ENGINE_LANG_MULTI_VALUE_SEPARATOR && !string_flag)
			return &(str[i]);
		//If string is open, then the new delimiter will close it
		else if (str[i] == OPH_QUERY_ENGINE_LANG_STRING_DELIMITER && string_flag)
			string_flag = 0;
	}
	return NULL;
}
...
int oph_query_parse_multivalue_arg(char *values, char ***value_list, int *value_num)
{
	...
	*value_list = NULL;
	...
	for (i = 0; values[i]; i++) {
		//In this case set the flag for an opening string delimiter
		if (values[i] == OPH_QUERY_ENGINE_LANG_STRING_DELIMITER && !string_flag)
			string_flag = 1;
		//Count separator only if not in the middle of a string
		else if (values[i] == OPH_QUERY_ENGINE_LANG_MULTI_VALUE_SEPARATOR && !string_flag)
			param_num++;
		//If string is open, then the new delimiter will close it
		else if (values[i] == OPH_QUERY_ENGINE_LANG_STRING_DELIMITER && string_flag)
			string_flag = 0;
	}
